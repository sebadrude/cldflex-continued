...
**Generated by [cldflex-continued](https://github.com/sebadrude/cldflex-continued), \
an independent continuation of [cldflex](https://github.com/fmatter/cldflex) \
originally created by Florian Matter.**

> **These CSVs are module-agnostic.** Run without the `--cldf` flag, the
> `dictionary` and `wordlist` commands produce the **same** intermediate tables
> documented below -- they are staging files, not a CLDF dataset. Add `--cldf`
> to additionally generate, under `cldf/`, a proper CLDF **Dictionary**
> (`EntryTable` + `SenseTable` + `ExampleTable`) or **Wordlist**
> (`FormTable` + `ParameterTable`) dataset; only then does the chosen module
> shape the output.
...
|------|-------------|-------------|
| `entries.csv` | lexical entry (headword) | `ID`, `Form`, `Gloss`, `Type`, `Parameter_ID` (its sense IDs), `Language_ID` |
| `senses.csv` | sense of an entry | `ID`, `Name`, `Description`, `Entry_ID` (-> `entries.csv`) |
| `lexemes.csv` | lexeme (root/stem entry) | as `entries.csv`, plus `Lexeme_ID` |
| `stems.csv` | stem form / allomorph | `ID`, `Form`, `Type`, `Lexeme_ID`, `Main_Stem` |
| `morphemes.csv` | morpheme (abstract) | `ID`, `Form`, `Gloss`, `Type` |
| `morphs.csv` | morph (surface allomorph) | `ID`, `Form`, `Type`, `Morpheme_ID` |

## Column conventions

* `form_<lang>` -- form/headword in the object language `<lang>`.
* `gloss_<lang>`, `definition_<lang>` -- gloss/definition in analysis language `<lang>`.
* `note_<type>_<lang>`, `literal-meaning_<lang>`, `reversal_<lang>` -- further FLEx fields.
* `relation_*` -- lexical relations and variant links exported by FLEx.
* `variant_*` -- variant / allomorph information.
* Cells with multiple values are separated by `"; "` (configurable via `csv_cell_separator`).
* A `<lang>|` prefix on a sense `Name`/`Description` (e.g. `pt|dar`) marks a sense whose
  meaning falls back to a **non-primary** analysis language -- i.e. an incomplete entry.
"""


def write_intermediate_readme(output_dir):
    """Write a README describing the module-agnostic intermediate CSVs, making
    clear they are identical for the dictionary and wordlist modules and are
    produced by cldflex-continued (crediting the original cldflex)."""
    dump(INTERMEDIATE_README, Path(output_dir) / "README.md")
...
def convert(
    ...
    if output_dir:
        ...
        write_intermediate_readme(output_dir)
        if not cldf and cldf_mode in ("dictionary", "wordlist"):
            log.warning(
                f"'{cldf_mode}' without --cldf produced module-agnostic "
                "intermediate CSVs (identical for the dictionary and wordlist "
                f"modules), not a CLDF {cldf_mode.capitalize()} dataset. Add "
                "--cldf for a proper CLDF dataset. See the generated README.md."
            )
    if cldf:
